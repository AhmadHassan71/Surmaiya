...
class SignUpActivity : AppCompatActivity() {
    ...
    private fun initializeViews() {
        ...
        passwordEditText.tooltipText =
            "Password must be greater than 8 characters, must have one upper case, one special character, and have a numeric value"
    ...
    private fun setUpOnClickListeners() {
        ...
        signUpButton.setOnClickListener {
            ...
            if (!isSignUpInProgress) {
                ...
                verifyFields {
                    if (it) {
                        FirebaseDatabaseManager.getInstance().signUpUser(user,this){
                            if(it){
                                sendPhoneVerificationCode(phoneNumberEditText.text.toString())
                                showOtpDialog()
                            }else{
                                CustomToastMaker().showToast(this, "Error creating user")
                            }
                        }
    ...
    fun setPasswordStrengthColor(passwordStrength: Int) {
        when (passwordStrength) {
            in 0..2 -> passwordStrengthProgressBar.progressTintList =
                ColorStateList.valueOf(Color.parseColor("#FF0000")) // Red color
            in 3..4 -> passwordStrengthProgressBar.progressTintList =
                ColorStateList.valueOf(Color.parseColor("#FFFF00")) // Yellow color
            5 -> passwordStrengthProgressBar.progressTintList =
                ColorStateList.valueOf(Color.parseColor("#008000")) // Green color
        }
    }
    ...
    private fun setPasswordListener() {
        ...
        passwordEditText.setOnTouchListener { v, event ->
            ...
            if (event.action == MotionEvent.ACTION_UP) {
                val touchArea =
                    passwordEditText.right - passwordEditText.compoundDrawables[DRAWABLE_RIGHT].bounds.width() - dpToPx(
                        this,
                        20
        ...
        confirmPasswordEditText.setOnTouchListener { v, event ->
            ...
            if (event.action == MotionEvent.ACTION_UP) {
                val touchArea =
                    confirmPasswordEditText.right - confirmPasswordEditText.compoundDrawables[DRAWABLE_RIGHT].bounds.width() - dpToPx(
                        this,
                        20
                    ) // 20 is the additional touch area in dps
                if (event.rawX >= touchArea) {
